...
class TestTagCopying(unittest.TestCase):
	...
	#def tearDown(self):
	#	# close the directory so it gets removed
	#	if self.temp_obj:
	#		self.temp_obj.close()
...
class TestExifToolAlpha(unittest.TestCase):
	...
	# ---------------------------------------------------------------------------------------------------------
	def setUp(self):
		self.et = exiftool.ExifToolAlpha(common_args=["-G", "-n", "-overwrite_original"], encoding='UTF-8')
	...
	def test_set_metadata(self):
		...
		for d in expected_data:
			...
			self.assertFalse(f_mod.exists(), f"{f_mod} should not exist before the test. Please delete.")
	...
	def test_set_keywords(self):
		...
		for d in expected_data:
			...
			source_files.append(f_mod)
			with self.et:
				self.et.set_keywords(f_mod_str, exiftool.experimental.KW_REPLACE, d["Keywords"])
